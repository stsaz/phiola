...
static void* fmtr_open(phi_track *t)
{
	...
	if (ffstr_eqz(&ext, "aac") && t->conf.stream_copy) {
		ffstr fn = FFSTR_INITZ(t->conf.ofile.name);
		ffstr_rsplitby(&fn, '.', NULL, &ext);
		if (ffstr_ieqz(&ext, "aac")) // .aac -> .aac copy
...
static const char* fmtr_hdr(struct fmt_rd *f, phi_track *t, struct avpk_info *hdr)
{
	...
	t->audio.bitrate = hdr->real_bitrate;
...
static const phi_filter fmt_read = {
